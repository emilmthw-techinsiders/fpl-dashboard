...
export function mergeBestXI(sourceLists, lookup, { budgetCap = 100, teamCap = 3 } = {}) {
  ...
  // Tries real fallback candidates for `pos` (or any of `positions`) best
  // real epNext first, not cheapest first — a forced fill is still a real
  // squad slot that has to play actual minutes, so it should be the best
  // affordable option, not the rock-bottom cheapest name that happens to
  // fit. `tryAdd` itself still enforces the real affordability/reserve
  // check, so this just changes which order options are tried in.
  const tryAddBestFiller = (positions) => {
    const ranked = allPlayers
      ...
      .sort((a, b) => num(b.epNext) - num(a.epNext));
