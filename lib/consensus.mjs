...
export function mergeBestXI(sourceLists, lookup, { budgetCap = 100, teamCap = 3, scoutPicks = null, teamNews = null, fallbackPool = [] } = {}) {
  ...
  // Real false-positive bug caught live (2026-08-26): the old second
  // condition (does the FULL scout name contain the candidate's own
  // webname?) let a short/generic webname wrongly match ANY scout pick
  // whose full name happened to contain that word — confirmed concretely:
  // "James Tarkowski" was matching webname "James" (a completely
  // different real player, Reece James/Daniel James also share that
  // webname), never actually reaching the real Tarkowski entry. Matching
  // on the scout name's surname against the candidate's own webname alone
  // handles every real case correctly (single-name entries like "Gabriel"
  // included, since splitting a one-word name just returns itself) without
  // this false-positive class.
  const scoutBonus = (p) => {
    if (!scoutPicks?.picks?.length) return 0;
    const pn = normalizeName(p.name);
    const hit = scoutPicks.picks.find((sp) => {
      const spLast = normalizeName(sp.name).split(' ').pop();
      return pn.includes(spLast);
    });
  ...
  while (upgraded && budgetCap - budgetUsed > 0.1) {
    upgraded = false;
    // Only ever touches fillers/weak single-source picks (votes < 2) — a
    // genuine 2/3 or 3/3 cross-source consensus pick is real signal this
    // loop has no business overriding just to spend leftover budget, even
    // if a pricier alternative would technically fit.
    const worstFirst = [...chosen].filter((c) => c.votes < 2).sort((a, b) => (a.votes - b.votes) || (num(a.player.epNext) - num(b.player.epNext)));
    for (const current of worstFirst) {
      ...
      const fitsGap = (p) => p.position === pos && !chosenNames.has(p.name)
      ...
      // Real gap caught live (2026-08-31): allPlayers here is scoped to
      // already-nominated candidates (widePool/bench/differentials/
      // budgetSafetyNet) — genuinely correct for the MAIN vote-based picks
      // (a random unvetted player shouldn't win a slot just because votes
      // ran out), but too narrow specifically for closing a small residual
      // budget gap: confirmed concretely, a real £0.3m leftover had zero
      // valid same-position replacement in this pool even though several
      // genuinely existed at the right price in the full player list
      // (fallbackPool) — the site was left £0.3m under the mandatory-full
      // £100m rule purely because none of those options had happened to
      // get pooled elsewhere. Only reached when the narrow pool comes up
      // empty, so this doesn't relax anything about the main picks.
      let replacement = allPlayers.filter(fitsGap).sort((a, b) => num(b.epNext) - num(a.epNext))[0];
      if (!replacement) {
        replacement = fallbackPool.filter(fitsGap).sort((a, b) => num(b.epNext) - num(a.epNext))[0];
      }
