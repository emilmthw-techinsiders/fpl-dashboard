...
export function nextFixtureByTeam(teams, fixtures) {
  ...
  for (const fx of upcoming) {
    if (!map[fx.team_h]) {
      map[fx.team_h] = { opponent: teamsById[fx.team_a]?.short_name || '?', venue: 'H', kickoff: fx.kickoff_time };
    }
    if (!map[fx.team_a]) {
      map[fx.team_a] = { opponent: teamsById[fx.team_h]?.short_name || '?', venue: 'A', kickoff: fx.kickoff_time };
    }
  }
  return map;
}
...
export function playerCard(el, teamsById, nextFixtureMap = {}) {
  ...
  return {
    ...
    // Real cost cut (2026-08-23): a short numeric code instead of the full
    // ~65-char badge URL, reconstructed client-side (see teamBadgeUrl in
    // public/app.js) — this field alone used to repeat the same template
    // string ~600 times in allPlayers every single refresh tick.
    teamCode: team ? team.code : null,
    position: POSITION[el.element_type],
    positionId: el.element_type,
    // Same reasoning as teamCode above — see playerPhotoUrl in
    // public/app.js for the reconstruction.
    photoCode: el.photo.replace('.jpg', ''),
...
// Real FPL rules, not our own math — each of these has an official points
// value already computed or directly derivable from the raw stat: BPS's
// actual bonus payout comes from FPL's own `bonus` fixture stat (already
// tie-resolved, e.g. two players tied for 2nd both get 2 and nobody gets 1
// — re-deriving "top 3 BPS" ourselves would get that wrong), saves pay 1pt
// per 3 saves, and defensive contribution pays a flat 2pts once a player's
// position-specific threshold is met (DEF:10, MID/FWD:12 combined actions
// — see dcThreshold below; goalkeepers never earn this one at all).
function statPointsSuffix(identifier, value, elementType, bonusByElement, elementId) {
  if (identifier === 'bps') {
    const bonus = bonusByElement[elementId];
    return bonus ? ` +${bonus} bonus` : '';
  }
  if (identifier === 'saves') {
    const pts = Math.floor(value / 3);
    return pts > 0 ? ` +${pts} pt${pts > 1 ? 's' : ''}` : '';
  }
  if (identifier === 'defensive_contribution') {
    if (elementType === 1) return ''; // goalkeepers never earn this
    return value >= dcThreshold(elementType) ? ' +2 pts' : '';
  }
  return '';
}
...
function fixtureStatsTable(f, elementsById) {
  ...
  const bonusByElement = {};
  for (const e of [...(bonusStat?.h || []), ...(bonusStat?.a || [])]) bonusByElement[e.element] = e.value;
  ...
  for (const stat of f.stats || []) {
    ...
    const entries = raw.map((e) => {
      const el = elementsById[e.element];
      const suffix = statPointsSuffix(stat.identifier, e.value, el?.element_type, bonusByElement, e.element);
      return `${el?.web_name || '?'} (${e.value})${suffix}`;
    });
...
function pickOneGameweekFixtures(teamsById, elementsById, fixtures, gwId, previousFixturesById) {
  return fixtures
    .filter((f) => f.event === gwId)
    .sort((a, b) => new Date(a.kickoff_time) - new Date(b.kickoff_time))
    .map((f) => {
      // Real, growing CPU cost caught live (2026-08-23), directly relevant
      // to running a full 38-gameweek season without this tipping over
      // Cloudflare's CPU limit again: recomputing BPS/DEFCON/bonus sorting
      // for every started-or-finished fixture on EVERY 5-minute tick,
      // forever, means this cost only grows as the season progresses — by
      // GW38 that's ~380 fixtures reprocessed every tick, even though a
      // TRULY finished fixture's stats (bonus points officially confirmed)
      // can never change again. Reuse the cached table from last run for
      // any fixture that was ALSO already finished then — guarantees the
      // cached value already reflects the fully-confirmed final numbers.
      // Only still-live fixtures, or one that just crossed into finished
      // since the last tick, get recomputed — keeping this cost roughly
      // constant (bounded by the CURRENT gameweek's fixtures) instead of
      // growing with total season history.
      const cachedFixture = previousFixturesById?.[f.id];
      const stats = (f.finished && cachedFixture?.finished && cachedFixture.stats)
        ? cachedFixture.stats
        : (f.started || f.finished) ? fixtureStatsTable(f, elementsById) : {};
      return {
        id: f.id,
        ...
        started: !!f.started,
        finished: f.finished,
        // Real FPL platform lag caught live (2026-08-22): the official
        // `finished` flag only flips once bonus points are fully confirmed,
        // which can take an hour+ after full time — `finished_provisional`
        // (true once the match itself has actually ended, minutes:90) is
        // what a "LIVE" badge should key off instead, or a match sitting in
        // that gap shows as LIVE long after it's actually over.
...
// previousAllGameweekFixtures: last run's own output of this same function
// (see the caching comment in pickOneGameweekFixtures above) — null on the
// very first run ever, which just means nothing to reuse yet.
function pickAllGameweekFixtures(teamsById, elementsById, fixtures, gameweeks, previousAllGameweekFixtures) {
  const previousFixturesById = {};
  for (const gw of previousAllGameweekFixtures || []) {
    for (const f of gw.fixtures || []) {
      if (f.id != null) previousFixturesById[f.id] = f;
    }
  }
  return gameweeks.map((gw) => ({
    event: gw.id,
    name: gw.name,
    fixtures: pickOneGameweekFixtures(teamsById, elementsById, fixtures, gw.id, previousFixturesById),
  }));
}
...
function pickSetPieceTakers(elements, teamsById) {
  ...
  // Plain comparison, not localeCompare — these are short ASCII team codes
  // (ARS, MUN, ...), so locale-aware collation buys nothing here, but its
  // FIRST call in a cold isolate pays real, non-trivial ICU initialization
  // cost. Confirmed live (2026-08-22): this one call alone accounted for the
  // majority of computeInsights' CPU time and was enough on its own to tip
  // the cron worker's scheduled invocation over Cloudflare's CPU limit,
  // silently failing every 5-minute tick with no visible site breakage
  // (old KV data just kept being served) until actually profiled.
  return Object.values(byTeam).sort((a, b) => (a.team < b.team ? -1 : a.team > b.team ? 1 : 0));
}
...
function pickPriceWatch(elements, teamsById, nextFixtureMap) {
  ...
  // Real bug caught live (2026-08-26): transfers_in_event/transfers_out_event
  // accumulate for the WHOLE gameweek and don't reset once a price actually
  // changes, so a player whose transfer momentum already triggered today's
  // real rise/fall (cost_change_event) kept showing up as "likely" to do
  // the exact same thing again, using the same stale count that already
  // cashed out — confirmed concretely: De Cuyper and M.Sangaré had both
  // already risen today (cost_change_event: 1) yet still topped "Likely
  // Risers" off the same momentum number. Excluding a riser candidate once
  // they've already risen today (and a faller once they've already fallen)
  // — real, already-happened Price Changes covers that case; this section
  // is specifically for what HASN'T happened yet.
  const risers = withDelta
    .filter((e) => e.netTransfers > 0 && e.cost_change_event <= 0)
...
// Real, already-happened price moves (FPL's own cost_change_event, £0.1m
// steps applied overnight) — distinct from Price Watch above, which is a
// PREDICTION based on this gameweek's transfer momentum, not a confirmed
// move yet. Both are useful for different reasons (Price Watch: "who
// might move soon", this: "who already moved") so both stay, as separate
// sections.
function pickPriceChanges(elements, teamsById, nextFixtureMap) {
  const active = elements.some((e) => e.cost_change_event !== 0);
  if (!active) return { active: false, risers: [], fallers: [] };

  const risers = elements
    .filter((e) => e.status === 'a' && e.cost_change_event > 0)
    .sort((a, b) => b.cost_change_event - a.cost_change_event)
    .slice(0, 5)
    .map((e) => ({ ...playerCard(e, teamsById, nextFixtureMap), costChangeEvent: e.cost_change_event }));

  const fallers = elements
    .filter((e) => e.status === 'a' && e.cost_change_event < 0)
...
//
// `skipXIComputation`: real, confirmed production bug (2026-08-23) — the
// scheduled cron refresh was silently failing EVERY 5-minute tick with
// Cloudflare's CPU limit (confirmed via wrangler tail: cpuTime 10ms on a
// 10ms budget). buildGameweekSquad's own score/sort pass over the full
// player pool was the single largest piece of that cost — and refresh.mjs
// (see runExpensiveAI there) ALWAYS immediately discards and overwrites
// bestXI/differentials with either the frozen locked pick or the previous
// snapshot whenever the AI consensus isn't due to run this tick, which is
// the overwhelming majority of ticks once a gameweek locks (which it does
// for hours/days at a stretch). Recomputing a full squad build for a
// result that gets thrown away unread, every 5 minutes, was pure waste.
// When true, buildGameweekSquad/pickDifferentials are skipped entirely in
// favor of cheap placeholders — safe specifically because the caller only
// ever sets this when it already knows it's about to overwrite both
// fields regardless of what they contain.
export function computeInsights(bootstrap, fixtures, overrideGwId = null, eloByCode = null, statsAgg = null, skipXIComputation = false, previousAllGameweekFixtures = null) {
  ...
  const EMPTY_BEST_XI = { formation: '0-0-0', rows: { gkp: [], def: [], mid: [], fwd: [] }, bench: [], captain: null, captaincyPicks: [], totalCost: 0 };
  return {
    isPreseason,
    ...
    bestXI: skipXIComputation ? EMPTY_BEST_XI : buildGameweekSquad(bootstrap.elements, teamsById, nextFixtureMap, fixturesMap, targetGwId),
    setPieceTakers: pickSetPieceTakers(bootstrap.elements, teamsById),
    chipWindows: pickChipWindows(bootstrap.teams, fixtures),
    differentials: skipXIComputation ? [] : pickDifferentials(bootstrap.elements, teamsById, nextFixtureMap, 8),
    ...
    priceChanges: pickPriceChanges(bootstrap.elements, teamsById, nextFixtureMap),
    ...
    allGameweekFixtures: pickAllGameweekFixtures(teamsById, elementsById, fixtures, gameweeksList, previousAllGameweekFixtures),
