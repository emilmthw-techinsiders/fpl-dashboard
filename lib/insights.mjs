...
function ceilingScore(el) {
  ...
  return threatPer90 * 0.1 + bonusPer90 * 2 + xgiPer90 * 5 + dcPer90 * 0.25;
}
...
//
// Real production bug found via GW1 (2026-08-21): the old filter required
// RELIABLE_MINUTES on its own, which silently hid a player from Gemini and
// Groq entirely if their past-minutes sample was thin — even when FPL's own
// forward-looking ep_next rated them the single best option at their
// position league-wide (verified: Raya, epNext 4.0, the highest of any
// keeper in the game, missing from the wide pool while sitting right there
// in the data). buildGameweekSquad's own deterministic vote is NOT gated
// this way (score() weighs ep_next directly, unaffected by past minutes),
// so a player like this could win the FPL Data vote on merit while being
// invisible to the other two voters — a solo correct pick outvoted 2-1 by
// two sources that never got the chance to consider it. Now included via
// EITHER real established minutes OR a genuinely strong forward-looking
// projection (ep_next >= 3, comfortably above an ordinary squad player but
// below what only a handful of the league's best carry) — a thin-minutes
// player still needs a real, meaningful signal to get in, this isn't just
// removing the filter.
const WIDE_POOL_CAPS = { 1: 4, 2: 12, 3: 14, 4: 8 };
const STRONG_EP_NEXT = 3;
export function buildWidePool(elements, teamsById, nextFixtureMap, fixturesByTeamIdMap, gwId) {
  ...
  const eligible = elements.filter((e) => e.status === 'a' && (num(e.minutes) >= RELIABLE_MINUTES || num(e.ep_next) >= STRONG_EP_NEXT));
  const byPos = { 1: [], 2: [], 3: [], 4: [] };
  for (const e of eligible) byPos[e.element_type].push(e);

  const pool = [];
  for (const pos of Object.keys(WIDE_POOL_CAPS)) {
    // Established (real minutes) and thin-minutes-but-strong-projection
    // players are sorted SEPARATELY, then combined — sorting everyone by
    // minutes alone (the old approach) would just push a Raya-type player
    // back to the bottom of the list and off the cap regardless of the
    // eligibility fix above, since low minutes always loses that sort.
    const established = byPos[pos]
      .filter((e) => num(e.minutes) >= RELIABLE_MINUTES)
      .sort((a, b) => num(b.minutes) - num(a.minutes));
    const thinButStrong = byPos[pos]
      .filter((e) => num(e.minutes) < RELIABLE_MINUTES)
      .sort((a, b) => num(b.ep_next) - num(a.ep_next));
    // A small guaranteed reserve for the strong-epNext group even when
    // established players fill the rest of the cap (so an exceptional
    // thin-minutes standout doesn't always get crowded out later in the
    // season) — but takeEstablished.length, not a fixed reserve subtracted
    // up front, decides how much cap is actually left for thinButStrong.
    // Real bug caught testing this against live GW1 data: early in a new
    // season EVERYONE'S minutes reset to near-zero, so `established` can be
    ...
    // from thinButStrong when established comes up short, and stays a
    // small reserve once established has plenty of players again.
    const minReserve = Math.min(2, thinButStrong.length, Math.floor(WIDE_POOL_CAPS[pos] / 4) || 1);
    ...
    const combined = [...takeEstablished, ...thinButStrong.slice(0, remainingCap)];
