...
// Real preseason friendly form (goals/assists — see public/friendlies.json)
// as a genuine input to the deterministic scoring and the Gemini/Groq
// prompts, not just chat/Compare Players trivia — pre-season, FPL's own
// form/ep_next/ict_index are all near-zero or last season's carryover, so a
// player's actual observed current form is otherwise invisible to Best
// XI/Differentials/AI Recommendation entirely. Callers are responsible for
// only passing `friendlies` through when isPreseason is true (see
// computeInsights) — this function itself doesn't gate on that, it just
// returns null if there's no real match, so it's inert once real season
// data exists and this naturally stops being called with real friendlies.
export function findFriendlyStats(playerName, friendlies) {
  if (!friendlies?.visible) return null;
  ...
  const scorer = (friendlies.topScorers || []).find((p) => nameMatches(p.name));
  const assister = (friendlies.topAssists || []).find((p) => nameMatches(p.name));
...
// Modest relative to ep_next/xGI (the proven anchors) — this sharpens
// ranking among similar players using real observed current form, the same
// "small relative weight" philosophy already used for defensive_contribution
// above, not a wholesale override of the underlying stats-based scoring.
function friendlyScoreBonus(el, friendlies) {
  const stats = findFriendlyStats(el.web_name, friendlies);
  if (!stats) return 0;
  return stats.preseasonGoals * 0.4 + stats.preseasonAssists * 0.25;
}
...
function score(el, friendlies = null) {
  return num(el.ep_next) * 3 + num(el.form) * 0.4 + num(el.points_per_game) * 0.15 + num(el.ict_index) * 0.01
    ...
    + friendlyScoreBonus(el, friendlies) + ownershipBonus(el);
}
...
function ceilingScore(el, friendlies = null) {
  ...
  // Weighted 1.5x here vs score()'s 1x — a hot preseason scoring streak is
  // itself an explosive-upside signal, arguably more relevant to "ceiling"
  // than to steady baseline projection.
  return threatPer90 * 0.1 + bonusPer90 * 2 + xgiPer90 * 5 + dcPer90 * 0.25 + friendlyScoreBonus(el, friendlies) * 1.5;
}
...
function buildGameweekSquad(elements, teamsById, nextFixtureMap, fixturesByTeamIdMap, gwId, friendlies = null) {
  ...
  const scored = pool
    .map((e) => ({ el: e, sc: score(e, friendlies) * gwMultiplier(e.team) }))
    .sort((a, b) => b.sc - a.sc);
  ...
  const captaincyScoreOf = (s) => s.sc * 0.7 + ceilingScore(s.el, friendlies) * 0.3;
  const rankedXI = startXI.slice().sort((a, b) => captaincyScoreOf(b) - captaincyScoreOf(a));
  const captain = rankedXI[0];
  const toCards = (list) => list.map((s) => playerCard(s.el, teamsById, nextFixtureMap));
...
function fixtureStatsTable(f, elementsById) {
  ...
  for (const stat of f.stats || []) {
    ...
    if (!label) continue;
    const entries = [...(stat.h || []), ...(stat.a || [])]
      .filter((e) => e.value)
      .map((e) => `${elementsById[e.element]?.web_name || '?'} (${e.value})`);
...
function differentialScore(el, friendlies = null) {
  return score(el, friendlies) * 0.6 + ceilingScore(el, friendlies) * 0.4;
}

export function pickDifferentials(elements, teamsById, nextFixtureMap, limit = 8, friendlies = null) {
  return elements
    .filter((e) => e.status === 'a' && num(e.selected_by_percent) < 10 && num(e.minutes) > 0)
    .sort((a, b) => differentialScore(b, friendlies) - differentialScore(a, friendlies))
    .slice(0, limit)
    .map((e) => ({
...
// position to keep prompt size sane; RELIABLE_MINUTES filters out small-
// sample noise the same way the scoring engine already does.
const WIDE_POOL_CAPS = { 1: 4, 2: 12, 3: 14, 4: 8 };
export function buildWidePool(elements, teamsById, nextFixtureMap, fixturesByTeamIdMap, gwId, friendlies = null) {
  // fixturesByTeamIdMap's own `difficulty` is already Elo-blended at the
  ...
  const eligible = elements.filter((e) => e.status === 'a' && num(e.minutes) >= RELIABLE_MINUTES);
  const byPos = { 1: [], 2: [], 3: [], 4: [] };
  for (const e of eligible) byPos[e.element_type].push(e);

  const pool = [];
  for (const pos of Object.keys(WIDE_POOL_CAPS)) {
    const sorted = byPos[pos].sort((a, b) => num(b.minutes) - num(a.minutes));
    for (const e of sorted.slice(0, WIDE_POOL_CAPS[pos])) {
      const card = { ...playerCard(e, teamsById, nextFixtureMap), fixtureDifficulty: gwDifficulty(e.team) };
      // Attaches real preseason friendly form for Gemini/Groq's own
      // independent analysis — same reasoning as chat/Compare Players
      // already had: pre-season, form/totalPoints are near-zero or last
      // season's carryover, so this is otherwise the only real "current
      // form" signal that exists.
      const friendlyStats = friendlies ? findFriendlyStats(card.name, friendlies) : null;
      pool.push(friendlyStats ? { ...card, preseasonFriendlyStats: friendlyStats } : card);
    }
  }
  return pool;
}
...
export function buildBudgetSafetyNet(elements, teamsById, nextFixtureMap, perPosition = 5) {
  ...
  for (const pos of Object.keys(byPos)) {
    ...
    for (const e of cheapest) pool.push(playerCard(e, teamsById, nextFixtureMap));
  }
  return pool;
}
...
export function compactPoolForPrompt(pool) {
  return pool.map((p) => ({
    ...
    ...(p.preseasonFriendlyStats ? { preseasonFriendlyStats: p.preseasonFriendlyStats } : {}),
  }));
}
...
// Scout Picks article each gameweek (see public/scoutPicks.json — same
// hand-updated, zero-API-cost pattern as friendlies.json; the actual PL news
...
export function computeInsights(bootstrap, fixtures, overrideGwId = null, friendlies = null, eloByCode = null, statsAgg = null) {
  ...
  // Real preseason form only matters before real season data exists — once
  // isPreseason flips false (permanently, for the rest of the season, see
  // lib/refresh.mjs's own reasoning on this), this becomes null and every
  // downstream score()/ceilingScore() call is unaffected, with zero extra
  // logic needed on their end.
  const activeFriendlies = isPreseason ? friendlies : null;
  ...
  return {
    ...
    bestXI: buildGameweekSquad(bootstrap.elements, teamsById, nextFixtureMap, fixturesMap, targetGwId, activeFriendlies),
    setPieceTakers: pickSetPieceTakers(bootstrap.elements, teamsById),
    chipWindows: pickChipWindows(bootstrap.teams, fixtures),
    differentials: pickDifferentials(bootstrap.elements, teamsById, nextFixtureMap, 8, activeFriendlies),
