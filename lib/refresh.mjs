...
import { refreshH2HFixtures, fetchH2HStandings } from './h2h.mjs';
...
export async function runRefresh(env) {
  ...
  // Real team-strength signal (see lib/eloRatings.mjs) — LAST season's
  // final ratings, which are frozen for the whole current season by
  // definition (the comment in eloRatings.mjs explains why). Refetching an
  // unchanging file on every 5-minute tick, forever, is pure waste — cached
  // in its own KV key, fetched at most once (or again only if a previous
  // attempt genuinely failed to return anything).
  const cachedEloRaw = await env.FPL_KV.get('eloRatings');
  let eloByCode = cachedEloRaw ? JSON.parse(cachedEloRaw) : null;
  if (!eloByCode) {
    eloByCode = await fetchEloRatings();
    if (eloByCode) await env.FPL_KV.put('eloRatings', JSON.stringify(eloByCode));
  }
  ...
  // empty array (zero network calls).
  //
  // Cached in its OWN KV key, not merged into the main 'latest' blob —
  // deliberately, so this cache can grow by one gameweek's worth of compact
  // per-player numbers every week without making the JSON.stringify cost
  // of the main blob (rewritten on EVERY 5-minute tick) grow right along
  // with it. Read cheaply every tick; only written back on the rare tick
  // where a gameweek has genuinely just finished for the first time (see
  // fetchPlayerStatsAgg's `changed` flag) — most ticks touch this key not
  // at all.
  const finishedGwIds = bootstrap.events.filter((e) => e.finished).map((e) => e.id);
  const previousStatsByGwRaw = await env.FPL_KV.get('playerStatsByGw');
  const previousStatsByGw = previousStatsByGwRaw ? JSON.parse(previousStatsByGwRaw) : null;
  const { agg: statsAgg, statsByGw: playerStatsByGw, changed: statsByGwChanged } = await fetchPlayerStatsAgg(finishedGwIds, previousStatsByGw);
  if (statsByGwChanged) {
    await env.FPL_KV.put('playerStatsByGw', JSON.stringify(playerStatsByGw));
  }

  // Real, confirmed fix (2026-08-23): whenever runExpensiveAI is false, the
  // branch below (or the hard final guard further down) ALWAYS overwrites
  // insights.bestXI/differentials wholesale with either the frozen locked
  // pick or the previous snapshot — buildGameweekSquad's own computation
  // is never read in that case. Skipping it there is what actually got the
  // scheduled refresh back under Cloudflare's CPU limit (see
  // computeInsights' own comment on skipXIComputation for the full story).
  const insights = computeInsights(bootstrap, fixtures, activeGwId, eloByCode, statsAgg, !runExpensiveAI, previous?.allGameweekFixtures);
  ...
  // stats.
  //
  // Real gap caught live (2026-08-28): this used to share runExpensiveAI's
  // gate with the Best XI/Differentials consensus, which is exactly right
  // for THOSE (they must never change once locked) but wrong for Team
  // News — alreadyLocked flips true the moment the gameweek's FIRST
  // deadline passes ('live' phase), even though most teams' own fixtures
  // within that gameweek haven't happened yet and their injury/lineup
  // news is still genuinely live information worth refreshing. Confirmed
  // concretely: GW2 went live and Team News froze for hours with a team
  // (Arsenal) whose own match was still 3 days out. Runs on its own
  // independent 3-hour throttle now, continuing until the WHOLE gameweek
  // is actually finished (activeEntry.status === 'complete'), not just
  // until its first deadline passes.
  const gwFullyFinished = activeEntry?.status === 'complete';
  const lastTeamNewsRunAtRaw = await env.FPL_KV.get('lastTeamNewsRunAt');
  const msSinceLastTeamNewsRun = lastTeamNewsRunAtRaw ? Date.now() - new Date(lastTeamNewsRunAtRaw).getTime() : Infinity;
  const runTeamNewsAI = !gwFullyFinished && msSinceLastTeamNewsRun >= AI_REFRESH_INTERVAL_MS;
  ...
  if (runTeamNewsAI) {
    ...
    const freshTeamNews = await fetchTeamNewsAI(env.GEMINI_API_KEY, teamNewsFacts);
    // Real hardening while touching this code: a transient failure (missing
    // key, network error, quota) used to silently WIPE teamNews to null
    // instead of keeping the last known-good content — exactly the
    // "shouldn't flicker empty" failure this section's own comment already
    // warns about, just not actually guarded for the case where the call
    // is attempted but fails, only the case where it's skipped entirely.
    if (freshTeamNews) teamNews = freshTeamNews;
    await env.FPL_KV.put('lastTeamNewsRunAt', new Date().toISOString());
  }
  ...
  // Pre-match odds don't meaningfully shift minute to minute — cached in
  // its own KV key and only refetched every few hours (same cadence
  // reasoning as the AI consensus throttle above), not on every tick.
  const WIN_PROB_REFRESH_INTERVAL_MS = 3 * 60 * 60 * 1000;
  const cachedWinProbRaw = await env.FPL_KV.get('winProbabilities');
  const cachedWinProb = cachedWinProbRaw ? JSON.parse(cachedWinProbRaw) : null;
  const winProbStale = !cachedWinProb || (Date.now() - new Date(cachedWinProb.fetchedAt).getTime()) >= WIN_PROB_REFRESH_INTERVAL_MS;
  let probabilityMap = cachedWinProb?.map || {};
  if (winProbStale) {
    const freshProbabilityMap = await fetchWinProbabilities(env.FOOTBALLDATA_API_KEY);
    if (Object.keys(freshProbabilityMap).length) {
      probabilityMap = freshProbabilityMap;
      await env.FPL_KV.put('winProbabilities', JSON.stringify({ map: probabilityMap, fetchedAt: new Date().toISOString() }));
    }
  }
  ...
  const mergedXI = mergeBestXI(xiSourceLists, lookup, { budgetCap: 100 - benchCost, teamCap: 3, scoutPicks, teamNews, fallbackPool: insights.allPlayers });
  ...
  // Inferno 14 H2H's own dedicated league table (W/D/L record, separate
  // from the classic league above) — see lib/h2h.mjs for why this can show
  // all-zero records mid-gameweek (FPL's own processing lag, not ours).
  const h2hStandings = await fetchH2HStandings();

  // Real gatekeeper (2026-08-23): allTeams/gameweeks/chipWindows/
  // setPieceTakers/fixturesByTeamId are genuinely static — they don't
  // change meaningfully within a day, let alone within 5 minutes — but
  // were being re-serialized and rewritten as part of the main blob on
  // EVERY tick regardless, all season. Split into their own KV key,
  // refreshed at most hourly (functions/get-data.js merges the two back
  // into the same response shape the frontend has always read, so this is
  // invisible to public/app.js). Deliberately does NOT include allPlayers
  // or the current gameweek's live fixtures — those genuinely need
  // every-tick freshness during live play, so splitting them out here
  // would just move the cost around, not reduce it.
  const STATIC_CATALOG_REFRESH_MS = 60 * 60 * 1000;
  const cachedCatalogRaw = await env.FPL_KV.get('staticCatalog');
  const cachedCatalog = cachedCatalogRaw ? JSON.parse(cachedCatalogRaw) : null;
  const catalogStale = !cachedCatalog || (Date.now() - new Date(cachedCatalog.fetchedAt).getTime()) >= STATIC_CATALOG_REFRESH_MS;
  if (catalogStale) {
    await env.FPL_KV.put('staticCatalog', JSON.stringify({
      allTeams: insights.allTeams,
      gameweeks: insights.gameweeks,
      chipWindows: insights.chipWindows,
      setPieceTakers: insights.setPieceTakers,
      fixturesByTeamId: insights.fixturesByTeamId,
      fetchedAt: new Date().toISOString(),
    }));
  }

  const { allTeams, gameweeks, chipWindows, setPieceTakers, fixturesByTeamId, ...liveInsights } = insights;
  await env.FPL_KV.put('latest', JSON.stringify({ ...liveInsights, ai, teamNews, history, h2hFixturesByGw, leagueStandings, h2hStandings, updatedAt: new Date().toISOString() }));
  return true;
}
