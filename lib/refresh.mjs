...
export async function runRefresh(env) {
  ...
  // Best-effort — cron-worker has no incoming request to build a relative
  // URL from (unlike functions/fpl-chat.js), so this hits the deployed site
  // directly. Never blocks the refresh; computeInsights only actually uses
  // it while isPreseason is true, and is a no-op otherwise regardless of
  // whether this fetch succeeds.
  let friendlies = null;
  try {
    const friendliesRes = await fetch('https://inferno-fpl.pages.dev/friendlies.json');
    if (friendliesRes.ok) friendlies = await friendliesRes.json();
  } catch (err) {
    console.error('friendlies.json fetch failed (non-fatal)', err.message);
  }
  ...
  // same best-effort, zero-cost fetch pattern as friendlies.json above. Only
  // used if its `gameweek` actually matches this refresh's active gameweek,
  // so a file that hasn't been updated yet for a new week is silently
  // ignored rather than fed in as if it were current.
  let scoutPicks = null;
  try {
  ...
  // Real team-strength signal (see lib/eloRatings.mjs) — unlike friendlies,
  // this isn't preseason-only: it's a genuine season-long read on team
  // quality, so it stays in play all year, not just gated to GW1.
  const eloByCode = await fetchEloRatings();

  // Real per-gameweek player stats (tackles/CBI/recoveries/DC-hits) — see
  // lib/playerStats.mjs. Best-effort, same as friendlies/elo: powers the
  // Player Statistics / Stats Per Gameweek pages, never blocks the refresh.
  // Only ever fetches gameweeks FPL itself already reports as finished —
  // pre-season that's an empty array (zero network calls), growing to the
  // real played gameweeks as the season progresses.
  ...
  const insights = computeInsights(bootstrap, fixtures, activeGwId, friendlies, eloByCode, statsAgg);
  ...
  // stats. One extra AI call per day, same non-fatal best-effort contract as
  // everything else here — a failure just means the tab shows its empty
  // state and the consensus prompts get no extra grounding, never a broken
  // refresh.
  const teamNewsFacts = buildTeamNewsFacts(bootstrap.elements, bootstrap.teams, nextFixtureMap);
  const teamNews = await fetchTeamNewsAI(env.GEMINI_API_KEY, teamNewsFacts);
  ...
  const widePool = buildWidePool(bootstrap.elements, teamsById, nextFixtureMap, insights.fixturesByTeamId, activeGwId, insights.isPreseason ? friendlies : null);
  ...
  const budgetSafetyNet = buildBudgetSafetyNet(bootstrap.elements, teamsById, nextFixtureMap);
  ...
  const mergedXI = mergeBestXI(xiSourceLists, lookup, { budgetCap: 100 - benchCost, teamCap: 3 });
  const { formation, rows } = groupIntoRows(mergedXI);

  // The bench above (insights.bestXI.bench) was computed by the deterministic
  // engine BEFORE the consensus XI existed — mergeBestXI's forced-fill safety
  // net (isFiller) can legitimately land on a real player who was ALSO
  // already sitting on that bench, producing the same person in both the
  // starting XI and the subs at once, which isn't a valid 15-man squad (a
  // real bug caught in production). Reconcile: drop any bench player who
  // ended up in the final XI, and backfill the vacated slot with a
  // different, real, affordable player at the same position so the bench
  // always stays a genuine, distinct 4.
  const mergedXINames = new Set(mergedXI.map((p) => p.name));
  const overlapping = insights.bestXI.bench.filter((p) => mergedXINames.has(p.name));
  let finalBench = insights.bestXI.bench.filter((p) => !mergedXINames.has(p.name));
  if (overlapping.length) {
    const usedNames = new Set([...mergedXINames, ...finalBench.map((p) => p.name)]);
    const teamCount = {};
    for (const p of [...mergedXI, ...finalBench]) teamCount[p.teamId] = (teamCount[p.teamId] || 0) + 1;
    for (const removed of overlapping) {
      const replacement = [...lookup.values()]
        .filter((p) => p.position === removed.position && !usedNames.has(p.name) && (teamCount[p.teamId] || 0) < 3)
        .sort((a, b) => parseFloat(a.price) - parseFloat(b.price))[0];
      if (!replacement) continue; // extreme edge case — leave bench one short rather than duplicate
